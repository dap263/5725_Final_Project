...
#define WRITE_BUF_SIZE  1 //2048
#define WRITE_BUF_TYPE  short
#define READ_BUF_SIZE 1
#define READ_BUF_TYPE unsigned short

#include <pigpio.h>

#define SPI_SS 26 // GPIO for slave select.
...
#define SAMPLES 200000000  // Number of samples to take,
...
int main(int argc, char *argv[])
{
   ...
   int pause;
	...
	int fdW, fdR, ret;
//	FILE *fp;
	char *writeFifo = "/home/pi/ECE-5725/Final-Project/adcBuf";
	char *readFifo = "/home/pi/ECE-5725/Final-Project/dacBuf";
   ...
   if (argc > 1) pause = atoi(argv[1]); else pause =0;
	...
	fdR = open( readFifo, O_RDONLY | O_NONBLOCK );
	if (fdR == -1) { printf("couldn't open read buf\n"); return 1; }
//	fp = fdopen(fdW, "w");
//	if (!fp) return 1; 
...
// MCP4822
	...
	#define DAC_MIN 1000
	#define DAC_MAX 4095
	short gainTarget = DAC_MAX; short oldGT = DAC_MAX;
	int h = spiOpen(1, SPEED, 0);
	if (h < 0) { printf("failed to open SPI 1\n"); return 1; }
	char spiBuf[2];
	unsigned short gain = 2100;
	unsigned short DAC_CTRL = 0x3000;

	spiBuf[0] = (DAC_CTRL | gain) >> 8;
	spiBuf[1] = gain & 0x00ff; 
	spiXfer(h, spiBuf, spiBuf, 2); // spiWrite(h, spiBuf, 2); // spi dac
   ...
   */
   ...
   while (sample<SAMPLES)
   {
      ...
      while (now_reading != reading)
      {
		...
		/*	if ( !(sample % 100) ) {
				spiBuf[0] = 0x3F; // spiBuf[0] = (DAC_CTRL | gain) >> 8;
				spiBuf[1] = 0x0F; // spiBuf[1] = gain & 0x00ff; 
				spiXfer(h, spiBuf, spiBuf, 2); // spiWrite(h, spiBuf, 2); // spi dac
			}*/
			if (bufPtr == WRITE_BUF_SIZE) {
				bufPtr = 0;
			// to send to another c file
				write(fdW, (void *) writeBuf, WRITE_BUF_SIZE*sizeof(WRITE_BUF_TYPE));
				ret = read(fdR, readBuf, READ_BUF_SIZE*sizeof(READ_BUF_TYPE));
				oldGT = gainTarget;
            if (ret > 0) 
					gainTarget = readBuf[0];

				if (gain > gainTarget) {
					gain--;
//					if (readBuf[0]) { // distortion detected
//						if (gain > DAC_MIN) gain--;
//					} else {
//						if (gain < DAC_MAX) gain++;
//					}	
//					printf("%d\n", readBuf[0]);
					spiBuf[0] = (DAC_CTRL | gain) >> 8;
					spiBuf[1] = gain & 0x00ff; 
               ...
               spiBuf[0] = (DAC_CTRL | gain) >> 8;
					spiBuf[1] = gain & 0x00ff; 
					spiXfer(h, spiBuf, spiBuf, 2);
            }
			// to send to python 
				//for (int idx = 0; idx < WRITE_BUF_SIZE; idx++) 
					//fprintf(fp, "%d\n", writeBuf[idx]);
			}
         //printf("\n");
