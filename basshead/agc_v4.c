...
//	printf("%d %d %d\n", bot,top,mid);
	...
	af[fPtr++ % R_SIZE] = lMax;
	lMax = AR_MAX(af);
	if ( (lMax > dnThresh) && (gainTarget != MIN_GAIN) && (width > (HIST_SIZE>>2)) && (cnt > 10 * INTERVAL) ) {
		upCnt = 0;
		printf("%6.4f\n", lMax);
		if (gainTarget <2200){
			gainTarget -= 100*log(dnCnt++); cnt = 0;
			printf("%d\n",dnCnt);
		} else {
			gainTarget -= 100; cnt = 0; //2 << (int)lMax; cnt = 0;
		}
		if (gainTarget < MIN_GAIN)
			gainTarget = MIN_GAIN;
	} else if ( (lMax <= upThresh || width < (HIST_SIZE>>2)) && (cnt > HOLD_TIME) ) {
		dnCnt = 0;
		//if (gainTarget < 2500) {
		//	gainTarget += 50*log(upCnt++); cnt = 0;
		//} else {
			gainTarget += 10; cnt = 0;
		//}
